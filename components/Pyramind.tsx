...
float cnoise(vec3 P){
  ...
  return 2.2 * n_xyz;
    ...
    void main() {
      float noise = cnoise(vPosition * 4.0 + uTime);
      vec3 green = vec3(1.0);
      vec3 blue = vec3(0.0, 0.0, 1.0);
      vec3 color = mix(green, blue, noise + 0.2);


  // Fresnel-like edge glow based on view angle
float fresnel = pow(1.0 - dot(normalize(vNormal), normalize(vViewDir)), 2.0);

  // Animate glow with time for natural movement
  ...
  vec3 glow = mix(blue, vec3(0.2, 0.8, 1.0), fresnel * anim);
    ...
    if (upFacing > 0.7) discard; // adjust threshold as needed
    gl_FragColor = vec4(glow, 0.4); // semi-transparent
  }
    }
`;
